...
static void clear_ucode_store(struct microcode_intel *ucode)
{
	ucode = NULL;
}
...
static int __is_lateload_safe(struct microcode_header_intel *mc_header)
{
	 ...
	 */
	if (override_minrev) {
		pr_warn("Bypassing minrev enforcement via debugfs\n");
		return 0;
	}
	...
	if (!mc_header->min_req_ver) {
		pr_warn("Late loading denied: Microcode header does not specify a required min"
			" version\n");
		return -EINVAL;
	}
